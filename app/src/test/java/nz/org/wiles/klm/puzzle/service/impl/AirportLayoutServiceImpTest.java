...
class AirportLayoutServiceImpTest {
    ...
    @Test
    void When_ModelSolution2IsConfigured_Then_LayoutIsAsExpected() {
        // setup
        int[] colFuelTruckCounters = {2, 1, 1, 2, 1, 1, 2};
        int[] rowFuelTruckCounters = {3, 1, 1, 2, 0, 2, 1};
        ...
        Plane[] planes = {
            ...
            Plane.builder().gridPos(new Point(1,4)).build(),
            Plane.builder().gridPos(new Point(1,6)).build(),
            Plane.builder().gridPos(new Point(2,3)).build(),
            Plane.builder().gridPos(new Point(2,6)).build(),
            Plane.builder().gridPos(new Point(4,1)).build(),
            Plane.builder().gridPos(new Point(5,3)).build(),
            Plane.builder().gridPos(new Point(6,0)).build(),
        ...
        assertEquals("E|E|E|P|E|F|P", Arrays.stream(airport.getGridLayout().getLayout()[2])
                                          .map(grid -> grid.getOccupationType().name().substring(0, 1))
                                          .collect(Collectors.joining("|")));

        assertEquals("E|F|E|F|E|E|E", Arrays.stream(airport.getGridLayout().getLayout()[3])
                                          .map(grid -> grid.getOccupationType().name().substring(0, 1))
                                          .collect(Collectors.joining("|")));
